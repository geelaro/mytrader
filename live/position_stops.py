...
Extracted from ``dashboard.signals.render_position_watch`` so the same
calculation can drive both the dashboard table and the daemon's
position-alert state machine.  Pure compute — no Streamlit or I/O beyond
calling :class:`DataProvider`.

Returned position dicts use the schema expected by
:func:`live.risk_alerts.RiskAlerter.check_positions`::
